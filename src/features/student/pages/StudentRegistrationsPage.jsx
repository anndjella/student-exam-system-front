...
function gradeOf(r) {
  const g = r?.grade ?? r?.Grade ?? r?.mark ?? r?.Mark ?? null;
  if (g === null || g === undefined) return null;
  const n = Number(g);
  return Number.isFinite(n) ? n : null;
}

function teacherFullNameOfReg(r) {
  const t = r?.teacherFullName ?? r?.TeacherFullName ?? null;
  console.log("active reg sample", r);
  ...
  return (
    <div className="container">
      <div className="page-header">
        ...
        <button className="btn" onClick={reloadAll} disabled={loading || actionLoading}>
          Refresh
        </button>
      </div>
              ...
              {loading ? (
                ...
                filteredSubjects.map(s => (
                  <tr key={subjectIdOf(s)}>
                    ...
                    <td className="page-subtitle">{teacherNames(s.teachers)}</td>
      ...
      {/* Active registrations */}
      <div className="card" style={{ padding: 12 }}>
        <div style={{ fontWeight: 900, marginBottom: 8 }}>My active registrations</div>
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th style={{ width: 110, textAlign: "center" }}>Grade</th>
                <th style={{ width: 160 }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                (activeRegs || []).map((r, idx) => {
                  const g = gradeOf(r);
                  const t = teacherFullNameOfReg(r);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="page-subtitle">{t ?? "-"}</td>

                      <td style={{ textAlign: "center" }}>{g == null ? "-" : g}</td>
